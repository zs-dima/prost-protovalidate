//! Descriptor-driven parity sweep.
//!
//! For every message in `PARITY_REGISTRY`, boundary vectors are generated
//! from the rules found in the descriptor itself (bounds ±1, lengths n−1/n/n+1,
//! const equal/different, `in`/`not_in` members and outsiders, NaN and
//! infinities for floats)
//! and each vector is validated through both the generated `Validate` impl
//! and the runtime `Validator`. **Agreement is the oracle** — the generator
//! carries no expected outcomes, so it can only be incomplete, never wrong.
//!
...
//! hand-pinned divergence between the two engines (see `tests/parity.rs`).
...
/// `gt_now`, `within`) evaluate identically across the two back-to-back
/// validation calls: 2100-01-01T00:00:00Z.
...
fn generate_vectors(desc: &MessageDescriptor) -> Vec<(String, DynamicMessage)> {
