//! Completeness gate: every message defined in the test protos must be wired
//! into `PARITY_REGISTRY` (has a generated `Validate` impl and is swept) or
//! explicitly listed in `ROUTED_TO_RUNTIME`. Adding a rule-bearing message to
//! `parity.proto` without wiring it into the parity infrastructure fails the
//! build.

use prost_protovalidate_tests::{FILE_DESCRIPTOR_SET_BYTES, PARITY_REGISTRY, ROUTED_TO_RUNTIME};
