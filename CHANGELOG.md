...
- **`bytes.pattern` on invalid UTF-8 (codegen path)**: when a `bytes` field carries a `pattern` rule and the value is not valid UTF-8, the generated `Validate` impl produces a `bytes.pattern` violation. The runtime `Validator` returns a `RuntimeError` for the same input instead — the `Validate` trait's `Result<(), ValidationError>` signature cannot carry a runtime error. This is the only intentional behavioural divergence between the two paths; valid-UTF-8 input is unaffected. Route bytes-pattern fields that need the runtime error surface through `Validator::validate` rather than the generated path.
